...
info %>%
 # dplyr::select(-Shoot_DM, -Bulb_DM) %>%
  ...
  ggplot(aes(x=Date,y=Value_mean, 
             shape=factor(Water_treat_level),
             linetype=factor(Water_treat_level),
             colour=factor(N_Treatments))) +
  geom_point() +
  geom_line() +
  facet_grid(Variable~Experiment, scales= "free") +
  ylab(" Intercepted  light (MJ/m2)  Total biomass (t DM/ha)") +
  xlab("Sampling date") +
...
#  mutate(N_treat_num = as.numeric(as.character(N_treat))) %>%
  filter(Exp != "W-") %>% # no N response in exp C
  group_by(Exp, N_treat, W_treat,W_treat_lev) %>%
  summarise_each(funs(mean, sd))
  
  
graphRUEnorm <- RUEnorm_df  %>% 
  ggplot(aes(x=as.numeric(as.character(N_treat)), y=as.numeric(RUEnorm_mean),
             colour=factor(W_treat))) +
  geom_point(aes(shape=factor(W_treat)),size = 4) +
 # geom_line() +
  geom_smooth(aes(linetype=factor(W_treat)),se = TRUE, method="lm") +
...
RUEnorm_df %>% 
  ungroup() %>%
  mutate(RUEnorm_mean = as.numeric(RUEnorm_mean)) %>%
  mutate(N_treat = as.numeric(as.character(N_treat))) %>%
  group_by(W_treat) %>%
...
info %>%
  ...
  summarise( HI_sd = sd(HI), HI = mean(HI)) %>%
#  summary() %>%
  ggplot(aes(x=Date, y=HI, linetype=Water_Treatments)) +
  geom_point(aes(shape=factor(N_Treatments), colour=factor(Water_treat_level))) +
  geom_smooth() +
  ...
  geom_errorbar(aes(ymin=HI-HI_sd/2,
                    ymax=HI+HI_sd/2), 
